...
Spatial polars can perform a [spatial join to join two dataframes](../SpatialFrame.md#spatial_polars.spatialframe.SpatialFrame.join) based on geomtric predicate, or [spatially join two lazyframes](../SpatialLazyFrame.md#spatial_polars.spatialframe.SpatialFrame.join) .   
...
```python title="Spatial Join" hl_lines="12-18"
--8<-- "spatial_join.py"
```

1. Reading the lakes
2. Reading the boundaries
...
Spatial joins can also be performed from LazyFrames as well, but tthey must be collected using the in-memory engine.
